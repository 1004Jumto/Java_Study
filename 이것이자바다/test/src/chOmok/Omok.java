...
public class Omok {
	...
	public static void main(String[] args) {
		...
		// 이름 중복시 임의로 숫자 붙여서 구분
		if(user.name.equals(computer.name)) computer.name += "2";
	...
	private static void play(Board board, Player user1, Player user2) {
		...
		while (!isStop) {
			...
			input = scanner.nextLine().replace("\t"," ").strip();
			point = input.split(" ");			// 공백을 기준으로 문자열 분리
			
			row =  Integer.parseInt(point[1]);	// "0"
			col = point[0].charAt(0) - 65; 		// "A"

			// 2. 입력 좌표 유효한지 확인 
			if (row > 18 || col > 18 || row < 0 || col < 0 || board.map[row][col] != ".") {
			...
			// 4. 오목 검사
			if (omokCheck(board, currentPlayer.stone, row, col)) {
				isStop = true;
				System.out.println("=====================================");
	...
	private static boolean omokCheck(Board board, String stone, int x, int y) {
		int cnt = 0;		// 연속되는 돌 개수
		int row, col;
